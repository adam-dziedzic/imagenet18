...
def get_loaders(traindir, valdir, sz, bs, fp16=True, val_bs=None, workers=8,
                rect_val=False, min_scale=0.08, distributed=False):
    val_bs = val_bs or bs
    train_tfms = [
    ...
    train_sampler = (
        DistributedSampler(train_dataset, num_replicas=env_world_size(),
...
def create_validation_set(valdir, batch_size, target_size, rect_val,
                          distributed):
    if rect_val:
        ...
        ar_tfms = [transforms.Resize(int(target_size * 1.14)),
                   CropArTfm(idx2ar, target_size)]
        val_dataset = ValDataset(valdir, transform=ar_tfms)
        val_sampler = DistValSampler(idx_sorted, batch_size=batch_size,
                                     distributed=distributed)
        return val_dataset, val_sampler

    val_tfms = [transforms.Resize(int(target_size * 1.14)),
...
class BatchTransformDataLoader():
    ...
    def process_tensors(self, input, target, non_blocking=True):
        ...
        if self.fp16:
            ...
            input = input.float()
        if len(input.shape) < 3: return input, target.cuda(
            non_blocking=non_blocking)
        return input.sub_(self.mean).div_(self.std), target.cuda(
            non_blocking=non_blocking)
    ...
    def __iter__(self):
        return (self.process_tensors(input, target, non_blocking=True) for
                input, target in self.loader)
...
class DistValSampler(Sampler):
    ...
    def __init__(self, indices, batch_size, distributed=True):
        ...
        self.expected_num_batches = math.ceil(
            len(self.indices) / self.world_size / self.batch_size)
