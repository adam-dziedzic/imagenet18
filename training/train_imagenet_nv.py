...
def get_parser():
    ...
    parser.add_argument('--fp16', action='store_true',
                        help='Run model fp16 mode. Default True')
    ...
    parser.add_argument('--auto-shutdown-success-delay-mins', default=10,
                        type=int,
                        help='how long to wait until shutting down on success')
    parser.add_argument('--auto-shutdown-failure-delay-mins', default=60,
                        type=int,
...
def main():
    ...
    best_top5 = 90  # only save models over this best_top5. Otherwise it stops to save every time
    ...
    if args.evaluate: return validate(dm.val_dl, model, criterion, 0,
                                      start_time)
    ...
    for epoch in range(args.start_epoch, scheduler.tot_epochs):
        ...
        if args.local_rank == 0:
            if is_best: save_checkpoint(
                epoch, model, best_top5, optimizer,
                is_best=True,
                filename=f'{args.save_path}/model_best.pth.tar')
            ...
            save_checkpoint(
                epoch, model, best_top5, optimizer,
                filename=f'{args.save_path}/checkpoint-epoch-{epoch}.pth.tar')
...
def train(trn_loader, model, criterion, optimizer, scheduler, epoch):
    ...
    for i, (input, target) in enumerate(trn_loader):
        ...
        reduced_loss, batch_total = to_python_float(loss.data), to_python_float(
            input.size(0))
        if args.distributed:  # Must keep track of global batch size, since not all machines are guaranteed equal batches at the end of an epoch
            metrics = torch.tensor(
                [batch_total, reduced_loss, corr1, corr5]).float().cuda()
        ...
        should_print = (batch_num % args.print_freq == 0) or (
                batch_num == len(trn_loader))
...
def validate(val_loader, model, criterion, epoch, start_time):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        timer.batch_start()
        if args.distributed:
            top1acc, top5acc, loss, batch_total = distributed_predict(input,
                                                                      target,
                                                                      model,
                                                                      criterion)
        else:
...
def distributed_predict(input, target, model, criterion):
    ...
    metrics = torch.tensor(
        [batch_size, valid_batches, loss, corr1, corr5]).float().cuda()
    batch_total, valid_batches, reduced_loss, corr1, corr5 = dist_utils.sum_tensor(
        metrics).cpu().numpy()
...
class DataManager():
    ...
    def preload_data(self, ep, sz, bs, trndir, valdir,
        ...
        if 'lr' in kwargs: del kwargs[
            'lr']  # in case we mix schedule and data phases
        ...
        if sz == 128:
            ...
            val_bs = max(bs, 128)
        return dataloader.get_loaders(trndir, valdir, bs=bs, val_bs=val_bs,
                                      sz=sz, workers=args.workers,
...
class Scheduler():
    ...
    def format_phase(self, phase):
        ...
        if len(phase['lr']) == 2:
            assert (len(phase[
                            'ep']) == 2), 'Linear learning rates must contain end epoch'
        return phase
    ...
    def linear_phase_lr(self, phase, epoch, batch_curr, batch_tot):
        ...
        return self.calc_linear_lr(lr_start, lr_end, ep_relative, batch_curr,
                                   ep_tot, batch_tot)

    def calc_linear_lr(self, lr_start, lr_end, epoch_curr, batch_curr,
                       epoch_tot, batch_tot):
    ...
    def get_lr(self, epoch, batch_curr, batch_tot):
        ...
        if len(phase['lr']) == 1: return phase['lr'][
...
def save_checkpoint(epoch, model, best_top5, optimizer, is_best=False,
...
def accuracy(output, target, topk=(1,)):
    ...
    return [correct_k.float().mul_(100.0 / batch_size) for correct_k in
            corrrect_ks]
...
if __name__ == '__main__':
    try:
        with warnings.catch_warnings():
            ...
            main()
        if not args.skip_auto_shutdown: os.system(
            f'sudo shutdown -h -P +{args.auto_shutdown_success_delay_mins}')
        ...
        if not args.skip_auto_shutdown: os.system(
            f'sudo shutdown -h -P +{args.auto_shutdown_failure_delay_mins}')
    tb.close()
